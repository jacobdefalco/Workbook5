...
function loadChoices() {
  categories.forEach((category) => {
    ...
    let categorySelection = new Option(category.name); //assigns categories to their own "category"
    selection.appendChild(categorySelection); //displays categories as options
  });
...
function buildActivityRow(tbody, activity) {
  ...
  cell2.innerText = "$" + activity.price.toFixed(2);

  let cell3 = row.insertCell(2);
  cell3.innerText = activity.location;

  let cell4 = row.insertCell(3);
  cell4.innerText = activity.id;
}
